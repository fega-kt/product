...
const { type } = require('@amcharts/amcharts4/core');
const dot = require('dot-object');
const moment = require('moment');
const { convertDot } = require('../helper');
...
function checkExistExcludeChar(strParent = "", str = "") {
  const listStrParent = strParent.split("")
  const listStr = str.split("")
  return listStrParent.some(r => listStr.includes(r))

}
function checkExistExcludeRegex(str1, str2) {
  const regex = new RegExp(str2);
  return regex.test(str1)
}
function checkString(str) {
  let newString = convertStringS(str)
  // const isNumeric = /^[a-zA-Z0-9"']+$/;
  const isNumeric = /^[a-zA-Z0-9 ]+$/;
  console.log(isNumeric, "isNumeric", str)
  return !isNumeric.test(newString)
}
...
function Validate(check, value) {
  ...
  if (checkedRequireForm) {
    ...
    if (type === 'String' && value && value.length && excludeRegex && excludeRegex.length && excludeRegexMsg && excludeRegexMsg.length && checkExistExcludeRegex(value, excludeRegex)) {
...
function viewConfigCheckForm(moduleCode, data) {
  try {
    ...
    let messages = {};
    if (checks && data) {
      // data = convertDot({ ob: data, newOb: { originItem: data } });

      Object.entries(checks).forEach(([configKey, check]) => {
        let newData = data
        if (check && check.type === "String" && check.name && check.name.includes(".")) {
          newData = convertDot({ ob: data, newOb: { originItem: data } });
        }
        messages[check.name] = Validate(check, newData[check.name]);
...
function getLogString(oldData, newData, code, kanban) {
  ...
  const content = `<div><span style="font-size: 13px;">${code === 'ST01' ? 'Cập nhật cơ hội kinh doanh' : code === 'ST03' ? 'Cập nhật trao đổi thỏa thuận' : 'Cập nhật công văn'
